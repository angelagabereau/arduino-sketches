...
void initSerialCommands(){
  ...
  sCmd.addCommand("DISPLAY_TEXT", cmdDisplayText);
...
void cmdDisplayText() {
  ...
  targetId = atoi(param1);
  while(param2 != NULL)      // As long as it existed, take it
  {
    text += param2;
    param2 = sCmd.next();
    // If there is more text to come, add a space.
    if(param2 != NULL){D
      text += " ";
    }
  }
